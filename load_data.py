...
def preprocess_keras(text):
    ...
    text_list = list(jieba.cut(str_no_punctuation))  # 分词列表
    text_list = [
        item for item in text_list if item != " " and item not in stopwords
    ]  # 去掉空格和停用词
    return " ".join(text_list)
...
def load_raw_datasets():
    ...
    tm_cost = t2 - t1
    print(
        "\nDone. {} total categories, {} total docs. cost {} seconds.".format(
            len(os.listdir(base_dir)), len(texts), tm_cost
        )
    )
    return texts, labels
...
def load_pre_trained():
    ...
    print(
        "Found %s word vectors, dimension %s" % (len(embeddings_index), embedding_dim)
    )
